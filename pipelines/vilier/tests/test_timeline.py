...
class TimelineTest(unittest.TestCase):
    ...
    def test_resolve_torch_device_prefers_cuda_then_cpu_for_auto(self):
        torch_module = SimpleNamespace(
            cuda=SimpleNamespace(is_available=lambda: False),
            backends=SimpleNamespace(mps=SimpleNamespace(is_available=lambda: True)),
        )
        self.assertEqual(resolve_torch_device(torch_module, "auto"), "cpu")
    ...
    def test_pyannote_diarizer_forwards_requested_speaker_count(self):
        calls = []
        ...
        diarizer = PyannotePixitDiarizer({"backend": "pyannote", "num_speakers": 2}, dry_run=True)
        diarizer.pipeline = pipeline
        with mock.patch("pipeline.diarization.pyannote_annotation_to_segments", return_value=[]):
            diarizer.diarize(Path("meeting.wav"), [])

        self.assertEqual(calls, [(("meeting.wav",), {"num_speakers": 2})])
    ...
    def test_diarizen_diarizer_loads_pipeline_and_normalizes_annotation(self):
        ...
        with (
            ...
            mock.patch("pipeline.diarization._patch_torchaudio_audio_metadata", new=lambda: None),
            mock.patch("pipeline.diarization._patch_pyannote_speaker_diarization_compat", new=lambda: None),
        ):
    ...
    def test_diarizen_diarizer_retries_from_pretrained_after_pyannote_audio_key_error(self):
        previous_pyannote_audio = sys.modules.get("pyannote.audio")
        class Pipeline:
            calls = []
        ...
        self.assertIs(sys.modules.get("pyannote.audio"), previous_pyannote_audio)
